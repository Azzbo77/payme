...
const CRYPTO_TICKERS: &[&str] = &[
    "BTC", "ETH", "XRP", "BCH", "LTC", "EOS", "XLM", "LINK", "DOT", "YFI", "DOGE", "ADA", "SOL",
    "MATIC", "AVAX", "LUNA", "ATOM", "NEAR",
];
...
fn validate_ticker(ticker: &str) -> Result<String, PaymeError> {
    ...
    if ticker_upper.len() < 2 || ticker_upper.len() > 6 {
        return Err(PaymeError::BadRequest(
            "Ticker must be 2-6 characters".to_string(),
        ));
    }
    ...
    for c in ticker_upper.chars() {
        if !c.is_alphanumeric() && c != '.' {
            return Err(PaymeError::BadRequest(
                "Ticker must contain only letters, numbers, and dots".to_string(),
            ));
        }
    }
...
pub async fn get_stock_price(
    ...
    let price =
        fetch_stock_price(&ticker, finnhub_key.as_deref(), alphavantage_key.as_deref()).await?;

    Ok(Json(StockPriceResponse { ticker, price }))
}
...
async fn fetch_stock_price(
    ...
    if let Some(key) = finnhub_key {
        match fetch_finnhub_price(&client, ticker, key).await {
            Ok(price) => {
                tracing::debug!(
                    "Successfully fetched {} price from Finnhub: {}",
                    ticker,
                    price
                );
                return Ok(price);
            ...
            Err(e) => {
                tracing::warn!(
                    "Finnhub lookup failed for {}: {}, attempting Alpha Vantage",
                    ticker,
                    e
                );
    ...
    if let Some(key) = alphavantage_key {
        ...
        if is_crypto(ticker) {
            match fetch_crypto_price(&client, ticker, key, base_url).await {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Crypto lookup failed for {}: {}, attempting stock lookup",
                        ticker,
                        e
                    );
...
async fn fetch_finnhub_price(
    ...
    let price = data.get("c").and_then(|v| v.as_f64()).ok_or_else(|| {
        PaymeError::BadRequest("Could not parse price from Finnhub response".to_string())
    })?;

    if price <= 0.0 {
        return Err(PaymeError::BadRequest(
            "Invalid price value from Finnhub".to_string(),
        ));
    }
...
async fn fetch_exchange_rate(from: &str, to: &str, api_key: &str) -> Result<f64, PaymeError> {
    let client = reqwest::Client::new();
    let base_url = "https://www.alphavantage.co/query";
